...
var palette=['8dd3c7', 'ffffb3', 'bebada', 'fb8072', '80b1d3', 'fdb462',
...
Entry.prototype.save = function(fn){
    ...
    CypherQuery.addStatement(user, hashtags, statement, contexts, mentions, gapscan, function(cypherRequest) {
        ...
        dbneo.cypherQuery(cypherRequest, function(err, cypherAnswer){
            ...
            if(err) {
                err.type = 'neo4j';
                console.log("Neo4j Error" + JSON.stringify(err));
                console.log("Query:" + cypherRequest);
                process.exit(444);
                return fn(err);
            }
...
Entry.getNodes = function(receiver, perceiver, contexts, fullview, showcontexts, res, req, fn){
   ...
   /* var querynodes =    "MATCH " +
        ...
        var querynodes =   "START rel=relationship(*) " +
        ...
        if(err) {
            err.type = 'neo4j';
            console.log("ENTRY.JS NEO4J ERROR");
            return fn(err);
        }

        var nodes_object = nodes.data;
        console.log("Nodes count=" + nodes_object.length);
...
//                    id: nodes_object[i][4],
                    ...
                    weight: parseFloat(nodes_object[i][7])
                });
  ...
  var community=jLouvain.community(g.nodes.map(function (obj){return obj.id}),g.edges);
 // console.log(community);
   
   if (community !== undefined) {
      var x;
      for (var j = 0; j < g.nodes.length; j++) {
            if (community[g.nodes[j].id]) {
                x=community[g.nodes[j].id];
                g.nodes[j].color='#' + palette[x % 12];
            } else g.nodes[j].color='#666';
        }
   console.log(g.nodes[0]);
   }
